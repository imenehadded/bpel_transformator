...
public class Bpel2Lts {
	...
	private String getInputFromFile()
		...
		/*
		 * File file = new File(this.bpel_file_name); String result = null;
		 * DataInputStream in = null;
		 * 
		 * try {
		 * 
		 * byte[] buffer = new byte[(int) file.length()]; in = new
		 * DataInputStream(new FileInputStream(file)); in.readFully(buffer);
		 * result = new String(buffer); in.close(); } catch (IOException e) {
	...
	public void exportLts(String output_file)
